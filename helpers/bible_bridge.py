    ...
    try:
        result = subprocess.run(
            [
                "gh", "pr", "create",
                "--title", pr_title,
                "--body", "Auto-generated by Fiction Writing Filter via /sync_pr",
                "--base", base_branch,
                "--head", branch,
            ],
    ...
    def do_POST(self):
        ...
        if parsed.path == "/bible/write":
            ...
            if err:
                self._send_json(400, {"error": err})
                return
            branch = body.get("branch", "fwf/" + time.strftime("%Y%m%d-%H%M%S"))
                ...
                try:
                    subprocess.run(
                        ["git", "config", "--global", "--add",
                         "safe.directory", project_path],
                        capture_output=True, timeout=5
                    )
                    log.info(f"Registered safe.directory: {project_path}")
                except Exception as e:
                    log.warning(f"Could not register safe.directory for {project}: {e}")
        else:
